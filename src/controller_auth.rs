use crate::{controller_accounts, controller_categories, controller_scheduled_transactions, jwt};
...
pub fn login(user: &NewAppUser) -> Option<InitialData> {
    if let Ok(app_user) = DatabaseAuth::new().authenticate_user(&user) {
        return Some(InitialData {
            ...
            scheduled_transactions: controller_scheduled_transactions::get_all_scheduled_transactions(app_user.id).expect("Error loading scheduled transactions"),
        });
    }
