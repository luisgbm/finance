...
use crate::models::{Account, AppUser, Category, CategoryTypes, NewAccount, NewAppUser, NewCategory, NewScheduledTransaction, NewTransaction, NewTransfer, ScheduledTransaction, Transaction, Transfer};
...
impl FinanceDB {
    ...
    pub fn get_all_scheduled_transactions(&self, app_user_id: i32) -> QueryResult<Vec<(ScheduledTransaction, Category, Account, AppUser)>> {
        ...
        scheduled_transactions::table.inner_join(categories::table).inner_join(accounts::table).inner_join(app_users::table)
            ...
            .order(date.asc())
            .load::<(ScheduledTransaction, Category, Account, AppUser)>(&self.connection)
    }
    ...
    pub fn get_scheduled_transaction(&self, find_id: i32, app_user_id: i32) -> QueryResult<(ScheduledTransaction, Category, Account, AppUser)> {
        ...
        scheduled_transactions::table.inner_join(categories::table).inner_join(accounts::table).inner_join(app_users::table)
            ...
            .first::<(ScheduledTransaction, Category, Account, AppUser)>(&self.connection)
    }
    ...
    pub fn update_scheduled_transaction(&self, update_id: i32, update_scheduled_transaction: &NewScheduledTransaction, app_user_id: i32) -> QueryResult<ScheduledTransaction> {
        ...
        diesel::update(scheduled_transactions.filter(user_id.eq(app_user_id)).find(update_id))
            .set((
                ...
                date.eq(update_scheduled_transaction.date),
            ...
            .get_result::<ScheduledTransaction>(&self.connection)
    }
