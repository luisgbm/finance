...
const IncomeCategoryOverTimeCategories = () => {
    const navigate = useNavigate();
    const dispatch = useDispatch();

    const categories = useSelector(state => state.categories);
    const selectedCategoryIds = useSelector(state => state.incomeCategoryOverTimeReport.selectedCategoryIds);

    // This report only ever charts income categories, so expense categories are not listed.
    ...
    const toggle = (id) => {
        ...
        dispatch({type: 'setIncomeCategoryOverTimeCategories', payload: allIds.filter(x => next.has(x))});
    };

    const selectAll = () => {
        dispatch({type: 'setIncomeCategoryOverTimeCategories', payload: [...allIds]});
    };

    const deselectAll = () => {
        dispatch({type: 'setIncomeCategoryOverTimeCategories', payload: []});
    };
...
export default IncomeCategoryOverTimeCategories;
