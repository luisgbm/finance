...
import {
    computeIncomeCategoryOverTime,
    computeForecastIncomeCategoryOverTime,
} from '../../utils/incomeCategoryOverTime';

const IncomeCategoryOverTime = () => {
    ...
    const {
        ...
        forecastCustomStart,
        forecastCustomEnd,
    } = useSelector(state => state.incomeCategoryOverTimeReport);
    ...
    const periodAction = isForecast ? 'setIncomeCategoryOverTimeForecastPeriod' : 'setIncomeCategoryOverTimePeriod';
    const customStartAction = isForecast ? 'setIncomeCategoryOverTimeForecastCustomStart' : 'setIncomeCategoryOverTimeCustomStart';
    const customEndAction = isForecast ? 'setIncomeCategoryOverTimeForecastCustomEnd' : 'setIncomeCategoryOverTimeCustomEnd';
    ...
    const onConfirm = async () => {
        const ids = selectedCategoryIds === null ? allIncomeIds : selectedCategoryIds;
        ...
        if (isForecast) {
            ...
            const data = computeForecastIncomeCategoryOverTime(scheduledTransactions, breakdown, range, selectedCategoryIds);
            navigate('/reports/income-category-over-time/chart', {state: {...data, subtitle: buildSubtitle()}});
            return;
        }
        ...
        try {
            ...
            const data = computeIncomeCategoryOverTime(transactions, breakdown, range, selectedCategoryIds);
    ...
    return (
        <>
            <AppBar position='sticky'>
                ...
                <Tabs
                    value={activeTab}
                    onChange={(e, v) => dispatch({type: 'setIncomeCategoryOverTimeTab', payload: v})}
                    centered
                >
            ...
            <Container maxWidth='sm' sx={{p: 3}}>
                <Card variant='outlined' sx={{mb: 3}}>
                    <List disablePadding>
                        <ListItemButton component={Link} to='/reports/income-category-over-time/categories'>
                ...
                <FormControl fullWidth variant='outlined' sx={{mb: 3}}>
                    ...
                    <Select
                        ...
                        label='Breakdown by'
                        value={breakdown}
                        onChange={e => dispatch({type: 'setIncomeCategoryOverTimeBreakdown', payload: e.target.value})}
                    >
                        {
...
export default IncomeCategoryOverTime;
