...
import {
    computeIncomesByCategory,
    computeForecastIncomesByCategory,
} from '../../utils/compareIncome';
...
const CompareIncome = () => {
    ...
    const {
        ...
        forecastCustomStart,
        forecastCustomEnd,
    } = useSelector(state => state.compareIncomeReport);
    ...
    const periodAction = isForecast ? 'setCompareIncomeForecastPeriod' : 'setCompareIncomePeriod';
    const customStartAction = isForecast ? 'setCompareIncomeForecastCustomStart' : 'setCompareIncomeCustomStart';
    const customEndAction = isForecast ? 'setCompareIncomeForecastCustomEnd' : 'setCompareIncomeCustomEnd';
    ...
    const onConfirm = async () => {
        ...
        if (isForecast) {
            ...
            const data = computeForecastIncomesByCategory(scheduledTransactions, range, selectedCategoryIds);
            navigate('/reports/compare-income/chart', {state: {...data, chartType, subtitle: buildSubtitle()}});
            return;
        }
        ...
        try {
            ...
            const data = computeIncomesByCategory(transactions, range, selectedCategoryIds);
    ...
    return (
        <>
            <AppBar position='sticky'>
                ...
                <Tabs
                    value={activeTab}
                    onChange={(e, v) => dispatch({type: 'setCompareIncomeTab', payload: v})}
                    centered
                >
            ...
            <Container maxWidth='sm' sx={{p: 3}}>
                <Card variant='outlined' sx={{mb: 3}}>
                    <List disablePadding>
                        <ListItemButton component={Link} to='/reports/compare-income/categories'>
                ...
                <FormControl fullWidth variant='outlined' sx={{mb: 3}}>
                    ...
                    <Select
                        ...
                        label='Chart type'
                        value={chartType}
                        onChange={e => dispatch({type: 'setCompareIncomeChartType', payload: e.target.value})}
                    >
