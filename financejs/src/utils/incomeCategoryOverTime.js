...
// Aggregate income transactions in the selected categories into per-bucket sums (integer
// cents) for the "Income Category Over Time" report. Only 'Income' transactions whose
// category is in selectedCategoryIds (null = all income categories) are counted; each bucket
...
export function computeIncomeCategoryOverTime(transactions, breakdown, range, selectedCategoryIds) {
    const selectedSet = selectedCategoryIds === null ? null : new Set(selectedCategoryIds);

    const inRange = transactions.filter((t) => {
        if (t.category_type !== 'Income') return false;
...
// Forecast variant: project scheduled income transactions in the selected categories into
// the range, then aggregate them per bucket exactly like the historical path. Only scheduled
// *Transactions* of type Income are counted (scheduled transfers are internal and excluded).
export function computeForecastIncomeCategoryOverTime(scheduledTransactions, breakdown, range, selectedCategoryIds) {
    const selectedSet = selectedCategoryIds === null ? null : new Set(selectedCategoryIds);
    const synthetic = [];
    ...
    return computeIncomeCategoryOverTime(synthetic, breakdown, range, selectedCategoryIds);
}
