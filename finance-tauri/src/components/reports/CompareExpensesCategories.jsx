...
const CompareExpensesCategories = () => {
    const navigate = useNavigate();
    const dispatch = useDispatch();

    const categories = useSelector(state => state.categories);
    const selectedCategoryIds = useSelector(state => state.compareExpensesReport.selectedCategoryIds);

    // This report only ever compares expense categories, so income categories are not listed.
    ...
    const toggle = (id) => {
        ...
        dispatch({type: 'setCompareExpensesCategories', payload: allIds.filter(x => next.has(x))});
    };

    const selectAll = () => {
        dispatch({type: 'setCompareExpensesCategories', payload: [...allIds]});
    };

    const deselectAll = () => {
        dispatch({type: 'setCompareExpensesCategories', payload: []});
    };
...
export default CompareExpensesCategories;
