...
pub async fn insert(pool: &SqlitePool, name: &str, user_id: i32) -> Result<Account, AppError> {
    let account = sqlx::query_as::<_, Account>(&format!(
        "INSERT INTO accounts (name, user_id) VALUES (?, ?) RETURNING {COLUMNS}"
    ))
    .bind(name)
    .bind(user_id)
    .fetch_one(pool)
...
pub async fn get_all(pool: &SqlitePool, user_id: i32) -> Result<Vec<Account>, AppError> {
    let accounts = sqlx::query_as::<_, Account>(&format!(
        "SELECT {COLUMNS} FROM accounts WHERE user_id = ? ORDER BY id"
    ))
    .bind(user_id)
    .fetch_all(pool)
    .await?;
...
pub async fn get(pool: &SqlitePool, id: i32, user_id: i32) -> Result<Account, AppError> {
    let account = sqlx::query_as::<_, Account>(&format!(
        "SELECT {COLUMNS} FROM accounts WHERE user_id = ? AND id = ?"
    ))
    .bind(user_id)
    .bind(id)
    .fetch_one(pool)
...
pub async fn update(
    ...
    let account = sqlx::query_as::<_, Account>(&format!(
        "UPDATE accounts SET name = ? WHERE user_id = ? AND id = ? RETURNING {COLUMNS}"
    ))
    .bind(name)
    .bind(user_id)
    .bind(id)
...
pub async fn delete(pool: &SqlitePool, id: i32, user_id: i32) -> Result<Account, AppError> {
    let account = sqlx::query_as::<_, Account>(&format!(
        "DELETE FROM accounts WHERE user_id = ? AND id = ? RETURNING {COLUMNS}"
    ))
    .bind(user_id)
    .bind(id)
    .fetch_one(pool)
...
pub async fn balance(pool: &SqlitePool, account_id: i32, user_id: i32) -> Result<i32, AppError> {
    ...
    let transfers_out: i64 = sqlx::query_scalar(
        "SELECT COALESCE(SUM(value), 0) FROM transfers WHERE origin_account = ? AND user_id = ?",
    )
    .bind(account_id)
    .bind(user_id)
    .fetch_one(pool)
    ...
    let transfers_in: i64 = sqlx::query_scalar(
        "SELECT COALESCE(SUM(value), 0) FROM transfers WHERE destination_account = ? AND user_id = ?",
    )
    .bind(account_id)
    .bind(user_id)
    .fetch_one(pool)
